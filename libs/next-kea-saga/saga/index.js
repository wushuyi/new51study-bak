...
export default {
  ...
  afterReduxStore: (options, store, cache) => {
  ...
  afterCreateSingleton: (input, output, cache) => {
    ...
    if (isActive) {
      ...
      output.saga = function * () {
        ...
        yield call(createCombinedSaga(sagas, sagaPath, cache))
      }
    }
  },

  injectToClass: (input, output, Klass, cache) => {
    if (output.activePlugins.saga) {
      injectSagasIntoClass(Klass, input, output, cache)
    }
  },

  injectToConnectedClass: (input, output, KonnektedKlass, cache) => {
    if (output.activePlugins.saga) {
      injectSagasIntoClass(KonnektedKlass, input, output, cache)
    }
  },
