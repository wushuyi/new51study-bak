...
function connectLogicIfUnconnected (logic, addReducer) {
  if (logic._isKeaSingleton && !logic._keaReducerConnected && logic.reducer) {
    addReducer(logic.path, logic.reducer)
    logic._keaReducerConnected = true
  }
}

export function deconstructMapping (mapping, addReducer) {
  ...
  for (let i = 0; i < mapping.length; i += 2) {
    ...
    connectLogicIfUnconnected(logic, addReducer)
