...
export function getStore (opts = {}, keaReducer, globalPlugins, activatePlugin, cache) {
  ...
  // run pre-hooks
  globalPlugins.beforeReduxStore.forEach(f => f(options, cache))
  ...
  // run post-hooks
  globalPlugins.afterReduxStore.forEach(f => f(options, store, cache))
