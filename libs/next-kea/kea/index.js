...
import { getStore as baseGetStore } from './store'
...
export default function createKea () {
  const {createActions, clearActionCache} = createAction()
  const cache = createCache()
  const {firstReducerRoot, isSyncedWithStore, addReducer, clearReducerCache, keaReducer} = createReducer()
  const {globalPlugins, activatePlugin, clearActivatedPlugins} = createPlugins()
  const {getCache, setCache, resetCache} = cache
  ...
  function kea (_input) {
    ...
    plugins.isActive.forEach(isActive => {
      output.activePlugins[isActive._name] = isActive(input, output, cache)
    })
    ...
    if (hasConnect) {
      ...
      output.connected = {
        actions: selectActionsFromLogic(connect.actions, addReducer),
        selectors: selectPropsFromLogic(connect.props, addReducer),
        propTypes: propTypesFromConnect(connect, addReducer)
      }
      ...
      plugins.afterConnect.forEach(f => f(input, output, cache))
    }
    ...
    if (output.isSingleton) {
      ...
      if (hasManualPath || input.reducers || input.selectors) {
        ...
        plugins.mutateReducerObjects.forEach(f => f(input, output, output.created.reducerObjects, cache))
        ...
        plugins.mutateReducer.forEach(f => f(input, output, output.reducer, cache))
      ...
      plugins.afterCreateSingleton.forEach(f => f(input, output, cache))
    }
    ...
    const response = function (Klass) {
      ...
      if (!isStateless(Klass)) {
        ...
        plugins.injectToClass.forEach(f => f(input, output, Klass, cache))
      }
        ...
        return (nextState, nextOwnProps) => {
          ...
          if (hasLogic) {
            ...
            if (!!getCache(joinedPath, reduxMounted) !== reduxMounted) {
              ...
              // run plugins on the created reducer objects
              plugins.mutateReducerObjects.forEach(f => f(input, output, reducerObjects, cache))
              ...
              if (shouldMountReducer && !reduxMounted) {
                ...
                // run plugins on the created reducer
                plugins.mutateReducer.forEach(f => f(input, output, reducer, cache))
      ...
      if (isStateless(Klass)) {
        plugins.injectToConnectedClass.forEach(f => f(input, output, KonnektedKlass, cache))
      }
    ...
    plugins.addToResponse.forEach(f => f(input, output, response, cache))
  ...
  const connect = (mapping) => kea({connect: mapping})

  function getStore (opts = {}) {
    return baseGetStore(opts, keaReducer, globalPlugins, activatePlugin, cache)
  }
